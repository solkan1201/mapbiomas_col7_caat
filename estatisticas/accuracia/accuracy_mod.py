...
output_dir = 'OUTPUT'
prefixo = "classesV4_norm_jan.csv"
pointsAcc = "occTabela_Caatinga_" + prefixo 
path_acc = 'acuracy_' + prefixo 
#version = 'v5'
STRATA_FILE =  'strataCaatinga.csv'

arq_acc = open(path_acc, 'w+')
...
def get_classes(df, level='l3'):
	...
	class_values = {}
	class_names = {}
	# reading class unique in dataframe
	clas_classes = pd.Index(df['classification'].unique())
	ref_classes = pd.Index(df['reference'].unique())
	# select class with value in reference class and classification class
...
def classification_report_shinny(df, class_names, class_values, year):	
	...
	texto = str(level) + ';' +  str('year')+ ';' + str(glob_acc*100) + ';' + str(quantity_dis_tot*100) + ';' + str(allocation_dis_tot*100)
	print(texto)
	...
	print('###', year, glob_acc, glob_se, )

	return result

def classification_report(df, class_names, class_values,  year):
	...
	footer2 = [year, 'Prop.area.se (Pontos)']
...
def calculate_prob(df):
	...
	df = pd.merge(df, strata, how='inner', on="bacia")  # "strata_id"
...
def accuracy_assessment(df, level='l3', year='Todos'):
	...
	resultados = classification_report_shinny(df, class_names, class_values,  year)
...
def population_estimation(df):
	...
	# num_occorrencias = int(df[['prob_amos']].to_numpy().flatten())

	sample_weight = 1 / df[['prob_amos']].to_numpy().flatten()
	return sample_weight.sum()# , num_occorrencias
...
print("list of columns the table is : \n ===> " , df_csv.columns)

df = calculate_prob(df_csv)

total_points = population_estimation(df)  # , number_total
print("total de pontos das amostras ", total_points)
